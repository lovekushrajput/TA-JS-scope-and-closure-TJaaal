...
function objOfMatchesWithArray(array1, array2, callback) {
  let obj ={}
for(let i=0 ; i<array1.length; i++){
 let val = callback.reduce((cv,elm)=> elm(cv),array1[i])
 if(val== array2[i]){
...
```js
function schedule(arr,arrScnd) {
for(let i=0;i<arr.length; i++){
const myTimeout = setTimeout(arr[i], arrScnd[i]*1000);
